...
import itertools
from sklearn.metrics import f1_score,matthews_corrcoef as mcc,fbeta_score,recall_score,precision_score
import math
import warnings
...
cols = [col for col in df.columns if ('label' in col) and col!='label_opinion']
df = df[cols]
...
def compute_krippendorff_alpha_for_k_runs(df, runs, k=None):
    ...
    for combination in tqdm(itertools.combinations(runs, k), total=math.comb(len(runs), k)):

    # for combination in itertools.combinations(runs,k):

        combination = list(combination)
        alpha_value = compute_krippendorff_alpha(df[combination]) if k!=1 else 0 
...
with concurrent.futures.ProcessPoolExecutor() as executor:
    for k in range(1, len(runs)+1,2):
        future = executor.submit(compute_krippendorff_alpha_for_k_runs, df, runs, k)
...
for result in results:
    if len(result)!=0:
        result.to_csv(f"agreement_analysis/{path}/{len(result['combination'][0])}.csv")
